...
class RecDataset(object):
    ...
    def load_inter_graph(self, file_name):
        ...
        self.sim_array = np.load(os.path.join(self.dataset_path+'/user_history_mm_pre.npy'))
    ...
    def split(self):
        ...
        # Top and Bottom User 
        N = len(self.sim_array)
        k = int(0.2 * N)  # 20%的数量
        sorted_indices = np.argsort(self.sim_array)
        # 后20%：相似度最低的（最小的前k个）
        low_indices = sorted_indices[:k]
        # 前20%：相似度最高的（最大的后k个）
        high_indices = sorted_indices[-k:]

        dfs.append(dfs[2][dfs[2]['userID'].isin(low_indices)])
        dfs.append(dfs[2][dfs[2]['userID'].isin(high_indices)])
