...
def compute_normalized_laplacian(adj):
    ...
    edge_values = adj.values()
    deg = scatter_add(edge_values, row, dim=0, dim_size=adj.shape[0])

    deg_inv_sqrt = deg.pow_(-0.5)
    deg_inv_sqrt.masked_fill_(deg_inv_sqrt == float('inf'), 0)
    edge_weight = deg_inv_sqrt[row] * edge_values * deg_inv_sqrt[col]
    return torch.sparse_coo_tensor(adj.indices(), edge_weight, adj.shape)
