...
class GRCN(GeneralRecommender):
    ...
    def calculate_loss(self, interaction):
        ...
        user_his_image_feat = self.aver_image_history_feat[batch_users]
        pos_item_image_feat = self.norm_raw_image_feat[interaction[1]]
        neg_item_image_feat = self.norm_raw_image_feat[interaction[2]]

        user_his_text_feat = self.aver_text_history_feat[batch_users]
        pos_item_text_feat = self.norm_raw_text_feat[interaction[1]]
        neg_item_text_feat = self.norm_raw_text_feat[interaction[2]]

        # item-to-history similarity
        user_his_sim_image = self.user_like_sim_image[batch_users]
        ...
        user_his_sim_text = self.user_like_sim_text[batch_users]
        ...
        stack_m_pre = torch.stack((pos_m_pre, neg_m_pre))
        m_pre_tensor = stack_m_pre.t().contiguous().view(-1)

        out = self.forward()
        user_score = out[user_tensor]
        item_score = out[item_tensor]

        score = torch.mul(F.elu(torch.sum(user_score * item_score, dim=1)) + 1, torch.pow(m_pre_tensor, self.gamma)).view(-1, 2) # [batch_size, 2]
    ...
    #     user_his_image_feat = user_his_image_feat_all[interaction[0]]
    #     user_his_text_feat_all = self.aver_text_history_feat
    #     user_his_text_feat = user_his_text_feat_all[interaction[0]]

    #      # user_similarity_preference
    #     user_his_sim_image = self.user_like_sim_image[interaction[0]]
    #     user_his_sim_text = self.user_like_sim_text[interaction[0]]
    ...
    #     score_matrix = (F.elu(torch.matmul(temp_user_tensor, item_tensor.t()) + 1)) * final_user_m_pre
    #     self.restore_user_e, self.restore_item_e = user_tensor, item_tensor
    #     return score_matrix
