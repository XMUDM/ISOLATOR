...
class GUME(GeneralRecommender):
    ...
    def bpr_loss_with_m_pre(self, users, pos_items, neg_items, pos_m_pre, neg_m_pre):
        pos_scores = torch.sum(torch.mul(users, pos_items), dim=1)
        neg_scores = torch.sum(torch.mul(users, neg_items), dim=1)
        
        pos_score_with_m_pre = torch.mul(F.elu(pos_scores) + 1  , torch.pow(pos_m_pre, self.gamma))
        neg_score_with_m_pre = torch.mul(F.elu(neg_scores) + 1 , torch.pow(neg_m_pre, self.gamma))
        
        regularizer = (self.sq_sum(users) + self.sq_sum(pos_items) + self.sq_sum(neg_items)) / self.batch_size

        maxi = F.logsigmoid(pos_score_with_m_pre - neg_score_with_m_pre)
        mf_loss = -torch.mean(maxi)
    ...
    def calculate_loss(self, interaction):
        ...
        # user_hisroty_item_feat
        user_his_image_feat_all = self.aver_image_history_feat
        user_his_image_feat = user_his_image_feat_all[users]
        pos_item_image_feat = self.norm_raw_image_feat[pos_items]
        neg_item_image_feat = self.norm_raw_image_feat[neg_items]

        user_his_text_feat_all = self.aver_text_history_feat
        user_his_text_feat = user_his_text_feat_all[users]
        pos_item_text_feat = self.norm_raw_text_feat[pos_items]
        neg_item_text_feat = self.norm_raw_text_feat[neg_items]

        # item-to-hisroty similarity
        ...
        bpr_loss, reg_loss_1 = self.bpr_loss_with_m_pre(u_g_embeddings, pos_i_g_embeddings,neg_i_g_embeddings, pos_m_pre, neg_m_pre)
    ...
    #     scores = (F.elu(torch.matmul(u_embeddings, restore_item_e.transpose(0, 1))) + 1) * final_user_m_pre
    #     return scores
    ...
    def full_sort_predict(self, interaction):
        ...
        self.restore_user_e, self.restore_item_e = restore_user_e, restore_item_e
        return scores
