...
class SLMRec(GeneralRecommender):
    ...
    def __init_weight(self, dataset):
        ...
        self.gamma_infer = self.config['gamma_infer']
        ...
        aver_image_his_feat_path = os.path.join(dataset_path, self.config['user_his_aver_image_feat'])
        aver_text_his_feat_path = os.path.join(dataset_path, self.config['user_his_aver_text_feat'])
        ...
        self.norm_raw_image_feat = F.normalize(self.v_feat, p=2, dim=1)
        self.norm_raw_text_feat = F.normalize(self.t_feat, p=2, dim=1)

        user_his_image_pre_path = os.path.join(dataset_path, self.config['user_his_image_pre'])
        user_his_text_pre_path = os.path.join(dataset_path, self.config['user_his_text_pre'])
    ...
    #     user_his_image_feat = user_his_image_feat_all[users]
    #     user_his_text_feat_all = self.aver_text_history_feat
    #     user_his_text_feat = user_his_text_feat_all[users]

    #     # user_similarity_preference
    #     user_his_sim_image = self.user_like_sim_image[users]
    #     user_his_sim_text = self.user_like_sim_text[users]
    #     match_t = torch.matmul(user_his_text_feat, self.norm_raw_text_feat.T)
    #     match_v = torch.matmul(user_his_image_feat, self.norm_raw_image_feat.T)
    ...
    #     scores = (F.elu(torch.matmul(users_emb, items_emb.t())) + 1) * final_user_m_pre
    #     return self.f(scores)
    ...
    def calculate_loss(self, interaction):
        ...
        user_his_image_feat_all = self.aver_image_history_feat
        user_his_image_feat = user_his_image_feat_all[users]
        pos_item_image_feat = self.norm_raw_image_feat[pos]
        neg_item_image_feat = self.norm_raw_image_feat[neg]

        user_his_text_feat_all = self.aver_text_history_feat
        user_his_text_feat = user_his_text_feat_all[users]
        pos_item_text_feat = self.norm_raw_text_feat[pos]
        neg_item_text_feat = self.norm_raw_text_feat[neg]
        ...
        main_loss = self.infonce_with_m_pre(users, pos, pos_m_pre, neg_m_pre)
    ...
    def infonce_with_m_pre(self, users, pos, pos_m_pre, neg_m_pre):
        (users_emb, pos_emb, neg_emb, userEmb0, posEmb0, negEmb0) = self.getEmbedding(users.long(), pos.long(), None)
        users_emb = torch.nn.functional.normalize(users_emb, dim=1)
        pos_emb = torch.nn.functional.normalize(pos_emb, dim=1)
        
        raw_logits = torch.mm(users_emb, pos_emb.T)
        logits_elu = F.elu(raw_logits) + 1.0
        
        pos_factor = torch.pow(pos_m_pre, self.gamma)  
        neg_factor = torch.pow(neg_m_pre, self.gamma) 

        batch_size = users_emb.shape[0]
        eye_mask = torch.eye(batch_size, device=self.device)

        logits_modified = logits_elu * (pos_factor * eye_mask + neg_factor * (1 - eye_mask))

        logits_modified /= self.temp

        labels = torch.arange(batch_size, device=self.device)
        return self.infonce_criterion(logits_modified, labels)
