...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def fit(self, train_data, valid_data=None, test_data=None, bottom_data=None, top_data=None, saved=False, verbose=True):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                valid_score, valid_result, _ = self._valid_epoch(valid_data)
                ...
                # test
                _, test_result, rec_list = self._valid_epoch(test_data)
                # bottom 
                _, bottom_result, rec_list = self._valid_epoch(bottom_data)
                # top 
                _, top_result, rec_list = self._valid_epoch(top_data)

                bias_result = {key: top_result[key] - bottom_result[key] for key in bottom_result}

                if verbose:
                    self.logger.info(valid_score_output)
                    self.logger.info(valid_result_output)
                    self.logger.info('test result: \n' + dict2str(test_result))
                    self.logger.info('bias result: \n' + dict2str(bias_result))
                if update_flag:
                    ...
                    self.best_bias_upon_valid = bias_result
                ...
                if stop_flag:
                    ...
                    break
        return self.best_valid_score, self.best_valid_result, self.best_test_upon_valid, self.best_bias_upon_valid
